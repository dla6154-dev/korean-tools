...
export default function StocksPage() {
  return (
    <div className="max-w-3xl mx-auto px-6 py-8">
      <h1 className="text-2xl font-bold text-slate-900 mb-1">📈 주식</h1>
      <p className="text-sm text-slate-500 mb-6">국내외 주식 시장 분석 및 투자 정보</p>
      <div className="space-y-4">
        {articles.stocks.map((article) => (
          <Link
            key={article.slug}
            href={`/stocks/${article.slug}`}
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <h2 className="text-base font-semibold text-slate-900 group-hover:text-blue-700 transition-colors mb-1">
                  {article.title}
                </h2>
